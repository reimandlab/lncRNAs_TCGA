...
cancer_data = canc_datas_pcgs[z] #cancers list and canc_datas list should be the same 

get_canc_data = function(dtt){
  #get cancer specific candidates 
 	k = which(str_detect(colnames(dtt), "ENSG"))

 	z = which(colnames(dtt) %in% c(sample(colnames(dtt)[k], 1000), "age_at_initial_pathologic_diagnosis", 
...
all_cancers_results = mclapply(setup_data, run_cv, mc.cores = 2L)

#all_cancers_results = llply(setup_data, run_cv, .progress="text")
saveRDS(all_cancers_results, file="PCGs__RANDOM_100_internal_CVs_individual_cands_june19.rds")
print("DONE SIR, all good WOO")

#get average and median c-index for each lncRNA clincial variabels per cancer type

#r = readRDS("PCGs_RANDOM_100_internal_CVs_individual_cands_june19.rds")
#r = ldply(r)
#r = as.data.table(r)
