...
z = which(colnames(canc_data) %in% c(lihc_features$V1, "canc", "time", "status", "sex", "patient"))
...
canc_data = canc_data[,which(colnames(canc_data) %in% c(lihc_features$V1, "time", "status"))]
...
pdf("timedependentAUC_externalPCAWG_LIHC.pdf")
...
pdf("topCands_LIHC_individual_survivaplots_TCGA.pdf")
...
pdf("topCands_LIHC_individual_survivaplots_PCAWG.pdf")
