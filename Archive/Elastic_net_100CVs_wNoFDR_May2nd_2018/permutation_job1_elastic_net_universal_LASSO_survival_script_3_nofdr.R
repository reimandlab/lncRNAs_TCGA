...
for(p in 1:5){
	...
	filename_cinds = paste(dat$Cancer[1], "cindices_permutations_1000CV_1000_no_fdr_ELASTICNET.rds", sep="_")
	saveRDS(all_cinds, file=filename_cinds)

	filename = paste(dat$Cancer[1], "lncRNAs_selected_permutations_1000CV_1000_no_fdr_ELASTICNET.rds", sep="_")
...
r = readRDS("Brain Lower Grade Glioma_lncRNAs_selected_permutations_1000CV_1000_no_fdr_ELASTICNET.rds")

rs = unlist(r)
rs = as.data.table(table(rs))
rs = rs[order(-N)]
