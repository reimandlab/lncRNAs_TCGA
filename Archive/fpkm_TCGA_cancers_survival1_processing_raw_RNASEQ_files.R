...
#--external dataset
external = readRDS("external_data_set_pcawg_patients_all_tumours_7501_tissues_TCGAnew.rds")
rownames(external) = external$gene
external$gene = NULL
external = t(external)
...
#z <- which(colnames(external) %in% ucsc$hg19.ensGene.name2)
#external = external[,z]
...
#external
z = which(colnames(external) %in% c(colnames(rna), colnames(pcg)))
external = external[,z]
...
rownames(external) = sapply(rownames(external), change)
...
z <- which(rownames(external) %in% clin$bcr_patient_barcode)
external = external[z,] #all have clinical data - 563 patients (matched normals)
...
#Add survival info to external file
external = as.data.frame(external)
external$patient = rownames(external)
external = merge(external, clin, by="patient")

saveRDS(external, "all_genes_external_tcga_all_cancers_March13_wclinical_data.rds")
