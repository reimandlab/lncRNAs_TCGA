...
get_canc = function(canc){
  ...
  z = which(!(str_detect(colnames(canc_data), "OS")))
  test = canc_data
...
random_permutations = function(canc){ #main permutation cross-validation function, which basically runs elastic net function on 100 random datasets 
  ...
  #check their prognostic capabilities within randomly generated dataset 
  ...
  if(!(length(z)==0)){
    ...
    #newdat$gene = factor(newdat$gene, levels = c(0,1))
   
    cox_mod = coxph(Surv(OS.time, OS) ~ gene, data = newdat)
    print(glance(cox_mod)$concordance)
    conc = round(glance(cox_mod)$concordance, digits=2)
    wald_p = glance(cox_mod)$p.value.wald
    hr = round(summary(cox_mod)$coefficients[2], digits=4)

    results = c(cancer, gene, canc, conc, wald_p, hr)
  ...
  perms_surv_results1 = perms_surv_results1[order(fdr)]
...
apply_to_all = function(list_cancers){
  res = llply(list_cancers, random_permutations, .parallel=TRUE)
  res = ldply(res)
  res$round = paste(sample(1:5000000, 1), sample(1:5000000, 1), sep="_")
  res$fdr = ""
  res$fdr = p.adjust(res$wald_p, method="fdr")
  res = as.data.table(res)
  res = res[order(fdr)]
  file = paste("feb22_EN_perms/", "round", res$round[1], ".rds", sep="_")
  saveRDS(res, file)
  print(paste("done", res$round[1]))
}


replicate(100, apply_to_all(cancers))

#all_cancers_all_perms = replicate(2, apply_to_all(cancers))

#saveRDS(all_cancers_all_perms, file="ALL_new_perms_0222.rds")
