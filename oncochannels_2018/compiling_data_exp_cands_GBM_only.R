...
kidata_sub = subset(kidata_sub, Cancer == "GBM")
...
#saveRDS(alldat, file="ion_channels_3_methods_survival_analysis_KI_April12_all_cancers.rds")
#write.csv(alldat, file="ion_channels_3_methods_survival_analysis_KI_April12_all_cancers.csv", quote=F, row.names=F)
...
write.csv(alldat, file="GBM_only_including_microarray_merged_pvalues.csv", quote=F, row.names=F)
...
#make heatmap
alldat = as.data.table(filter(alldat, fdr < 0.2, HR_median < 20, HR_outlier < 20))
table(alldat$Cancer)
...
date = Sys.Date()
pdf(paste(date, "summary_figure_ICs_gbm_only.pdf", sep="_"), width=12)
...
pdf("heatmap_only_ICs_gbm_only.pdf")
g = ggplot(alldat, aes(gene_name, Cancer)) + geom_tile(aes(fill=max_hr)) + theme_bw() + 
  scale_fill_gradient(low="grey", high="red", na.value = 'transparent') + labs(x = "Ion Channel", y="Cancer") #+ coord_flip()
...
date = Sys.Date()
write.csv(alldat, file=paste(date, "gbm_only_merged_pvals_all_cancers_hazardous.csv", sep="_"), quote=F, row.names=F)
...
library(plyr)
pdf(paste(date, "gbm_only_individual_cancers_barplots.pdf", sep="_"))
