...
add_tags = function(dtt){
  ...
  for(k in 1:length(medians)){
    ...
    dtt$gene = dtt[,z]
    dtt$gene = log1p(dtt$gene)
    dtt$med_tag = factor(dtt$med_tag, levels = c("Low", "High"))
    
    file = paste(names(medians)[k], dtt$type[1], "outlier split.png", sep="_")
    png(file, units="in", width=5, height=5, res=300)
    g = ggboxplot(dtt, "med_tag", "gene",
     fill = "med_tag", palette = c("#00AFBB", "#FC4E07"), title=paste(names(medians)[k], dtt$type[1], "outlier split"))+
    xlab("Outlier tag") + ylab("log1p(FPKM-UQ)") + stat_n_text()
    print(g)
    dev.off()
...
get_survival_models = function(dtt){
  ...
  file = paste(colnames(dat)[num_genes[i]], dtt$type[1], "outlier_split_km_plot.png", sep="_")
  png(file, units="in", width=5, height=5, res=300)
