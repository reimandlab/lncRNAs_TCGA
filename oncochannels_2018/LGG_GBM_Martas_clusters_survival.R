...
gbm_clusters$patient = sapply(gbm_clusters$patient, function(x){paste(unlist(strsplit(x, "\\.")), collapse="-")})

lgg_clin = as.data.table(filter(all, type == "LGG"))
gbm_clin = as.data.table(filter(all, type == "GBM"))

lgg_clin = unique(lgg_clin[,c("patient", "OS", "OS.time", "type")])
gbm_clin = unique(gbm_clin[,c("patient", "OS", "OS.time", "type")])

lgg_clin = merge(lgg_clin, lgg_clusters, by="patient")
gbm_clin = merge(gbm_clin, gbm_clusters, by="patient")
