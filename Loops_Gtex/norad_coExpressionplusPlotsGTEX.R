...
#(1) across all samples 
pdf("norad_coexpressedPCGS_GTEXsept6th.pdf", pointsize=8)
d <- as.data.frame(d)
...
pdf("norad_HIGHgroup_coexpressedPCGSsept6thGTEX.pdf", pointsize=8)
d <- as.data.frame(d)
...
#(2) across high lncRNA samples 
pdf("norad_LOWgroup_coexpressedPCGSsept6thGTEX.pdf", pointsize=8)
d <- as.data.frame(d)
