...
files = list.files(pattern = "elastic_net_cindices_100_may2_CV.rds")
...
pdf("cindices_results_elastic_net_100CVs_FDR0.2_cancers_May2nd.pdf", width=12, height=12)
...
files = list.files(pattern = "elastic_net_lncRNAs_selected_100_may2_CV.rds")

###change to be universal 

genes_keep = as.data.frame(matrix(ncol = 4)) ; colnames(genes_keep) = c("Geneid", "NumtimesChosen" , "Cancer", "GeneName")

pdf("summary_of_genes_results_elastic_net_100CVs_FDR0.2_cancers_May2nd.pdf")
...
pdf("summary_of_final_lncRNA_candidates_elastic_net_100CVs_FDR0.2_cancers_May2nd.pdf", width=14, height=11)
...
overg = overg +  theme(text = element_text(size=10),
        axis.text.x = element_text(angle=55, hjust=1, size=7)) +
          ggtitle("Summary of genes selected 100 CVs with FDR 0.2 after Pre-filtering")
...
gg = gg +  theme(text = element_text(size=10),
        axis.text.x = element_text(angle=90, hjust=1, size=6)) +
          geom_hline(yintercept = 50, linetype = 2, color = "blue") + ggtitle("Summary of genes selected 100 CVs with FDR 0.2")
...
#save final candidate lncRNAs 
saveRDS(genes_keep, file="genes_keep_100CV_FDR0.2_May2nd2018.rds")
