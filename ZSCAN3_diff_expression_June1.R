...
getPCGS <- function(df){
	...
	z = which(colnames(df) %in% c("patient", "ENSG00000189298"))
	rcc1 = df[,z]
	...
	df = merge(df, rcc1, by=c("patient", "ENSG00000189298"))
	...
	z = which(low %in% "ENSG00000189298")
	if(!(length(z) ==0)){
		low = low[-z]
	}
...
diffE <- function(d){
    ...
    #save top gene names 
    top <- c(paste("ENSG00000189298", d$type[1]), t$ID)
...
saveRDS(diffEresults1, file="32_TCGA_cancer_types_ZKSCAN3_diffExpressed_PCGs_June27.rds")
