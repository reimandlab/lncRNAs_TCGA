...
#tcga results
tcga = readRDS("survival_results_DEC15_liver_TCGA.rds")
...
#---------------------------------------------------------
tcga_genes = tcga$gene
tcga_genes = fantom$CAT_geneName[which(fantom$CAT_geneID %in% tcga_genes)]

lnc_rna <- lnc_rna[,c((which(colnames(lnc_rna) %in% tcga_genes)), 5608,5609)] #215 lncRNAs remain 
...
for(i in 1:length(tcga_genes)){
  #1. Subset lnc_rna to those patients in cancer
  df <- lnc_rna
  z <- which(colnames(df) %in% tcga_genes[i])
  df <- df[,c(z,20:24)]  
...
pdf("survival_results_using_median_lncRNAs_DEC15_tcga_valiation.pdf", pointsize=6, width=9, height=8)
require(gridExtra)
