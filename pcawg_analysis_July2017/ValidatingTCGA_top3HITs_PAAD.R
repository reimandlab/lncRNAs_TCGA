...
#subset to PAAD 
genes = subset(genes, canc == "pancreas")
...
for(i in 1:nrow(results_cox)){
  results_cox$name[i] = fantom$CAT_geneName[which(fantom$gene == results_cox$gene[i])]
}
...
for(i in 1:2){
  ...
  #cox
          ...
          s <- ggsurvplot(
          title = paste(gene, "HR =", round(results_cox$HR[i], digits=4), df$canc[1]),
          fit, 
          xlab = "Time (Years)", 
          surv.median.line = "hv",
