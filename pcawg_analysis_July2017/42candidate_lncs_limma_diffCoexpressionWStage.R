#42candidate_lncs_limma_diffCoexpressionWStage.R
...
diffE <- function(d){
    ...
    t <- as.data.table(t)
    #first by adj p values
    t <- t[order(adj.P.Val)]
    t <- t[order(-abs(as.numeric(logFC)))]

    #restrict to those with logFC greater than 75% 
    quant <- summary(t$logFC)[5]
    t <- t[abs(logFC) >= quant]
	...
	posPaths <- gprofiler(pos$ID, organism = "hsapiens", ordered_query= TRUE, min_set_size=35, max_set_size = 300, min_isect_size=10, correction_method="fdr")
	negPaths <- gprofiler(neg$ID, organism = "hsapiens", ordered_query= TRUE, min_set_size=35, max_set_size = 300, min_isect_size=10, correction_method="fdr")
...
saveRDS(diffExpressed, file="Sept6thupdated_42candidatesWithDEpcgsSept5.rds")
