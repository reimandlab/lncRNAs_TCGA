...
getPCGS <- function(df){
	all <- merge(df, pcg_rna, by=c("patient", "canc", "stage"))
	all <- all[,-(ncol(all))]
	colnames(all)[3] <- "stage"
	...
	all[,c(4:(ncol(all)))] <- log1p(all[,c(4:(ncol(all)))])
...
linear_regression <- function(column, d){
	...
	cancer <- d$canc
	#(1) across all samples
	lm0 <- lm(d[,column] ~ d$stage)
	lm1 <- lm(d[,column] ~ d$stage + d[,4])
	anov_p <- anova(lm0, lm1)[2,6]
	coef <- lm1$coefficients[4]
	coef_p <- summary(lm1)$coefficients[4,4]
	if((anov_p <= 0.05) & (coef_p <= 0.05)) {
		#plot regression
		fit <- lm(d[,column] ~ d$stage + d[,4])
		median <- median(as.numeric(fit$model[,3]))
		print(ggplot(fit$model, aes_string(x = names(fit$model)[3], y = names(fit$model)[1])) + 
  		geom_point() +
  		geom_vline(xintercept = median) + 
  		stat_smooth(method = "lm", col = "red") +
  		labs(title = paste("Adj R2 = ",signif(summary(fit)$adj.r.squared, 4),
                     "Intercept =",signif(fit$coef[[1]],4 ),
                     " Slope =",signif(fit$coef[[4]], 4),
                     " P =",signif(summary(fit)$coef[4,4], 4)), x=colnames(d)[4], y=gene))
...
#(1) across all samples 
pdf("WithStageasCofactor_LINC00665_coexpressedPCGSsept7th.pdf", pointsize=8)
d <- as.data.frame(d)
...
pos <- df[lm_coefficient > 0]
#pathway enrichment looking at top 100 coexpressed genes based on the above ranking
posGenes <- pos[1:200,1]
...
#combine 
allPathsNorad <- rbind(negNoradpaths, posNoradpaths)

#only keep GO or REACTOME
reac <- grep("REAC", allPathsNorad$GO.ID)
go <- grep("GO", allPathsNorad$GO.ID)
allPathsNorad <- allPathsNorad[c(reac, go), ]

write.table(allPathsNorad, sep= "\t", file=paste(colnames(d)[4], "UsedStageAllLINC00665TopPathsgProfiler.txt", sep="_"), quote=F, row.names=F)
