...
#Using same 1,604 genes but use their original FPKM and then log values
#for PCA plot 
top_genes <- rownames(floored_heatmap_matrix)
z <- which(rownames(lnc_rna_top5) %in% top_genes)
lnc_rna_top5_topgenes <- lnc_rna_top5[z,]
lnc_rna_top5_topgenes <- log1p(lnc_rna_top5_topgenes)

#Set up patient - cancer type list 
pats <- colnames(lnc_rna_top5)
z <- which(clin_top5$icgc_donor_id %in% pats)
canc <- as.factor(clin_top5$combined_tum_histo[z])

#pca
lnc_rna_top5_topgenes <- t(lnc_rna_top5_topgenes)
pca <- prcomp(lnc_rna_top5_topgenes)
library(ggbiplot)

ggbiplot(pca, obs.scale = 1, var.scale = 1,
  groups = canc, ellipse = TRUE, circle = TRUE, labels.size = 0.2) +
...
fviz_pca_ind(pca,
             ...
             habillage = canc, # color by groups
             palette = mypal,
