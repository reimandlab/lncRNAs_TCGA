...
colsskeep = c(which(str_detect(colss, "stage")), which(str_detect(colss, "grade")), which(str_detect(colss, "days")), 
  which(str_detect(colss, "age")), which(str_detect(colss, "followup")))
...
canc_data = canc_data[,-c(14,17)] #dont need cancer and sex columns
...
saveRDS(cindes_lncs_vs_clinical, file="cinds_LIHC_clinical_vs_all_lncRNAs_feb9_1000_runs.rds")
...
saveRDS(cindes_individual_lncs, file="cinds_LIHC_individual_lncRNAs_feb9_1000_runs.rds")
