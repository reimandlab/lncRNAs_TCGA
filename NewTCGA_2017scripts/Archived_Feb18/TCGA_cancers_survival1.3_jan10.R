...
make_density_plot = function(gene_data){
	...
	sp = ggplot(gene_data, aes(x=meanper, y=sdper)) + geom_point(alpha = 0.2, aes(colour = factor(type)))
...
#7. Change percentile cutoff, obtain list of candidate and do surival 
#analysis 
...
analyze_cands = function(dtt){
	
	genes = percentile_compare(dtt)
...
all_cancers_survival_results = llply(percentile_ranks, analyze_cands, .progress="text")
