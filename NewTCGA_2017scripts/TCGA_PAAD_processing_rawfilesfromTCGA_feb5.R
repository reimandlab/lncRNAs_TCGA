...
#4. RNA-Seq File
rna = readRDS("182rnaSEQfiles_PAAD.rds") 
...
rna$gene = rna[,1]
rna[,184] <- apply(rna[,183:184], 1, extract3) ; 
rna$V1 = NULL
...
colnames(rna)[1:182] = sapply(colnames(rna)[1:182], change)
...
cancers_order = unlist(llply(colnames(rna)[1:182], get_cancer))
...
lnc_rna = rna[z,]
saveRDS(lnc_rna, "5919_lncs_177_PAAD_patients_TCGAnew.rds")

#everything else (includes PCGS, other lncRNAs not in FANTOM)
pcg_rna = rna[-z,]
saveRDS(pcg_rna, "54564_pcgs_177_PAAD_patients_TCGAnew.rds")
