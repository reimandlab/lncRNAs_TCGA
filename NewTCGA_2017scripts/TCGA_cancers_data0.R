...
TCGA[,8975] <- apply(TCGA[,8974:8975], 1, extract3) ; 

#2. Seperate into cancers 

TCGA = as.data.table(TCGA)
...
cancers_order = unlist(llply(colnames(TCGA)[1:8974], get_cancer))
...
for(i in 1:nrow(cancers_keep)){
	source = unlist(strsplit(cancers_keep[i,1], '-'))[4]
...
z <- which(cancers_keep[,4] %in% dups)
...
#3. Subset RNA file 
z <- which(colnames(TCGA) %in% cancers_keep$TCGA_id)
