...
pdf("cindices_results_elastic_net_100CVs_No_FDR_cancers_May2nd.pdf", width=12, height=12)
...
files = list.files(pattern = "lncRNAs_selected_1000CV_1000_May1_no_prefiler.rds")

###change to be universal 

genes_keep = as.data.frame(matrix(ncol = 4)) ; colnames(genes_keep) = c("Geneid", "NumtimesChosen" , "Cancer", "GeneName")

pdf("summary_of_genes_lasso_net_1000CVs_No_FDR_cancers_May2nd.pdf")
...
pdf("summary_of_final_lncRNA_candidates_elastic_net_100CVs_No_FDR_cancers_May2nd.pdf", width=14, height=11)
...
overg = overg +  theme(text = element_text(size=10),
        axis.text.x = element_text(angle=55, hjust=1, size=7)) +
          ggtitle("Summary of genes selected 100 CVs with No FDR")
