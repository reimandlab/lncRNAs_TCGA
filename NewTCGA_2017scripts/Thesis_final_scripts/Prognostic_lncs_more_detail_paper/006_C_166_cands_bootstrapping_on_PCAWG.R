...
pdf("166_EXTERNAL_validation_unique_lncRNAs_cindices.pdf", width=10, height=6)
...
for(i in 1:length(unique(results$Cancer))){
	...
	meds = as.data.table(aggregate(canc_data$cindex, by=list(canc_data$lncRNA), FUN=median))
	...
	g = ggboxplot(canc_data, x="lncRNA", y="cindex", color="black", fill="type", palette=c("lavenderblush", "aliceblue")) + stat_compare_means() + 
	geom_label(data=meds, aes(x=lncRNA ,y = Median, label=Median), col='tomato', size=2)+
	xlab("Predictor")+ theme_bw() + ylim(c(0,1)) + 
	geom_hline(yintercept=0.5, linetype="dashed", color = "red") + stat_n_text() +
