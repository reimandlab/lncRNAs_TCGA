...
clin_subtypes = clin_subtypes[,c("patient", "Histology", "Grade", "Age..years.at.diagnosis." , "Gender",
...
get_km_plot = function(gene, cancer){
  ...
  dat = all[,c(which(colnames(all) %in% c("type", gene, "OS", "OS.time", "patient")))]
  ...
  dat = merge(dat, clin_subtypes, by="patient")
  ...
  dat$Original.Subtype = as.character(dat$Original.Subtype)
  cox_mod = coxph(Surv(OS.time, OS) ~ gene + Age..years.at.diagnosis. + MGMT.promoter.status + IDH.status, data = dat)
  cox_mod = coxph(Surv(OS.time, OS) ~ gene + MGMT.promoter.status, data = dat)

  print(glance(cox_mod)$concordance)
  conc = round(glance(cox_mod)$concordance, digits=2)
  fit <- survfit(Surv(OS.time, OS) ~ gene + MGMT.promoter.status, data = dat)
          s <- ggsurvplot(
