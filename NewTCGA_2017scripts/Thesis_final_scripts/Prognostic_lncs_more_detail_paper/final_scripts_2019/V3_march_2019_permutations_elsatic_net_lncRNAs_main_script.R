...
get_canc = function(canc){ #updated this function from old script 
  ...
  #canc_data$OS.time = NULL
    ...
    z = which(str_detect(colnames(canc_data), "ENSG"))
    #canc_data[,-z] = apply(canc_data[,-z], 2, sample)

    #clin data 
    clin = canc_data[,-z]

    #gene data 
    gene_dat = canc_data[,z]

    test = gene_dat[1:5,1:5]
    cols = colnames(test)
    test = matrix(sample(unlist(test)), ncol = ncol(test))
    colnames(test) = cols

    #shuffle all gene expression values across the board
    cols = colnames(gene_dat)
    gene_dat_new = matrix(sample(unlist(gene_dat)), ncol = ncol(gene_dat))
    colnames(gene_dat_new) = cols

    #combine wtih clin data 
    canc_data = cbind(gene_dat_new, clin)
...
source("survival_lncs_permutation_analysis_0214.R")
...
random_permutations = function(canc){ #main permutation cross-validation function, which basically runs elastic net function on 100 random datasets 
  ...
  #run elastic net cross-validation on dataset (n=1)
  #my.seed = 546
  #set.seed(my.seed, kind = "L'Ecuyer-CMRG" );

  print("start permutations")

  run_res = replicate(100, main_elastic_net(dato)) #DOUBLE CHECK number of replciations 
  ...
  dato$OS.time = as.numeric(dato$OS.time)
  dato$OS = as.numeric(dato$OS)
  ...
  #fit special elastic net - post selection inference 
  #we used elastic net to do feature selection 
  #so we can't just make inference based on full data because sample size is reduced 

  #get gene expression values for each gene 
  z = which(colnames(dato) %in% perms_surv_results1$gene)
  x = dato[,z]
  x = x[,perms_surv_results1$gene]

  label_pats = function(gene){
    z = which(colnames(x) == gene)
    med = median(x[,z])
    if(med == 0){
    l1 = which(x[,z] > 0)
    l2 = which(x[,z] == 0)
  ...
  x = sapply(colnames(x), label_pats)
  y = Surv(dato$OS.time, dato$OS)

  # first run glmnet
  gfit = glmnet(x,y,standardize=FALSE,family="cox", alpha =0.5)

  # extract coef for a given lambda; note the 1/n factor!
  lambda = 1.5
  n = nrow(x)
  beta = as.numeric(coef(gfit, s=lambda/n, exact=TRUE, x = x, y = y))
  #compute fixed lambda p-values and selection intervals
  out = fixedLassoInf(x, dato$OS.time, beta, lambda, status=dato$OS, family="cox", alpha=0.5)
  pvals = out$pv
  perms_surv_results1$inference_pvals = pvals

  file = paste("full_gene_exp_perm/", "round", canc, cohort_index, date, perms_surv_results1$round[1], "genes", ".rds", sep="_")
  ...
  file = paste("full_gene_exp_perm/", "round", canc, cohort_index, date, perms_surv_results1$round[1], "cindices", ".rds", sep="_")
...
#RUN 100 PERMUTATIONS OF 100 ELASTIC-NET CROSS VALIDATIONS 
