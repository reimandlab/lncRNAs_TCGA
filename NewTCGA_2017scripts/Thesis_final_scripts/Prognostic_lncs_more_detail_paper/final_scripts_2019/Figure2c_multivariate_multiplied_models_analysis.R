...
run_cv = function(dtt){
	...
	for(j in 1:1000){
		...
		#train and test model on each lncRNA candidate  
			...
			justlncs = coxph(Surv(OS.time, OS)  ~ . ^2, data = trainlncs)
	
			print("done lnc model")
...
saveRDS(all_cancers_results, file="lncRNAs_1000_internal_CVs_individual_cands_multivariate_multiplied_feb25.rds")

#get average and median c-index for each lncRNA clincial variabels per cancer type

r = readRDS("lncRNAs_1000_internal_CVs_individual_cands_multivariate_multiplied_feb25.rds")
r = ldply(r)
r = as.data.table(r)
