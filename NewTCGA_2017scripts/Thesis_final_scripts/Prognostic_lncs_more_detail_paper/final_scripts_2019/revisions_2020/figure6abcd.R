...
g = ggplot(all_res, aes(combo, num_sig_des, group=type)) + theme_classic() +
   geom_col(position = "dodge", aes(fill=type), color="grey29")+
	 xlab("lncRNA-cancer pair") + ylab("Number of significant \ngenes/pathways")+
   theme(legend.title=element_blank(), legend.position="top", axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))+ylim(c(0,2750))
print(g)
dev.off()
...
lncs_brain_all = unlist(canc[3,]$evidence)
...
res_all_brain$evidence = as.character(res_all_brain$evidence)
  ...
  dat_keep = dat[,which(colnames(dat) %in% c("patient", lncs_brain, pcgs_brain, lncs_brain_all))]
