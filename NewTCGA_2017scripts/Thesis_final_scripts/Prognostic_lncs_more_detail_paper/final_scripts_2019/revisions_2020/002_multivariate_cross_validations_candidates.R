...
get_canc = function(canc){
	...
	t=filter(as.data.table(table(canc_dat$histological_grade)), N <=5)$V1
	  print(t)
  	if((!(length(t)==0))){
    	z=which(canc_dat$histological_grade %in% t)
    	canc_dat = canc_dat[-z,]
  	}

	t=filter(as.data.table(table(canc_dat$clinical_stage)), N <=5)$V1
  	print(t)
  	if((!(length(t)==0))){
    	z=which(canc_dat$clinical_stage %in% t)
    	canc_dat = canc_dat[-z,]
  	}

  	t=filter(as.data.table(table(canc_dat$race)), N <=5)$V1
  	print(t)
  	if((!(length(t)==0))){
    	z=which(canc_dat$race %in% t)
    	canc_dat = canc_dat[-z,]
  	}
