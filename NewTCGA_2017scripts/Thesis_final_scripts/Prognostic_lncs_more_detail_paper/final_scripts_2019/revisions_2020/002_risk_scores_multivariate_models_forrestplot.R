...
run_cv = function(dtt){
	...
	dtt_model$risk[dtt_model$risk_score >median] = 1
	dtt_model$risk[dtt_model$risk_score <=median] = 0
	...
	lnc_cindices = c("all_lncs", dtt$Cancer[1], cindex_validation, "lncRNAonly", hr, pval, ci05, ci95)
	...
	justclin = coxph(Surv(OS.time, OS)  ~ ., data = clin_model)
	risk_scores = predict(justclin, newdata = clin_model, type="risk")

	#use risk scores to fit model
	clin_model$risk_score = risk_scores
	median= median(clin_model$risk_score)
	clin_model$risk[clin_model$risk_score >=median] = 1
	clin_model$risk[clin_model$risk_score <median] = 0
	justclin = coxph(Surv(OS.time, OS)  ~ risk, data = clin_model)

	# Determine concordance
	cindex_validation = glance(justclin)$concordance
	hr = summary(justclin)$coefficients[2]
	pval=summary(justclin)$coefficients[5]
   
   	ci05=summary(justclin)$conf.int[3]
   	ci95=summary(justclin)$conf.int[4]

	#which clinical variables were used
	variables_used = colnames(clin_model)[-which(colnames(clin_model) %in% c("OS", "OS.time"))]
	variables_used = paste(variables_used, collapse="--")

	clinical_cindices = c(variables_used, dtt$Cancer[1], cindex_validation, "ClinicalVariables", hr, pval, ci05, ci95)
	names(clinical_cindices) = c("clinVars", "Cancer", "Cindex", "type", "HR", "pval", "ci05", "ci95")

	clin_model$patient = rownames(clin_model)
	clin_model$risk = NULL
	clin_model$risk_score = NULL
	...
	lnc_clin_cindices = c("all_lncs", dtt$Cancer[1],cindex_validation, "lncRNA&clin", hr, pval, ci05, ci95)
...
saveRDS(all_cancers_results, file="lncRNAs_clinical_variables_risk_score_models_multivariate.rds")
