...
cancers = unique(r$canc)

get_canc = function(canc){
	canc_dat = subset(pcg, type == canc)
	return(canc_dat)
}
...
get_canc_data = function(dtt){
  ...
  z = which(colnames(dtt) %in% c(as.character(r$pcg[r$canc == dtt$type[1]]), "age_at_initial_pathologic_diagnosis",
...
saveRDS(all_cancers_results, file="nearby_pcgs_1000_internal_CVs_individual_cands_june19.rds")
