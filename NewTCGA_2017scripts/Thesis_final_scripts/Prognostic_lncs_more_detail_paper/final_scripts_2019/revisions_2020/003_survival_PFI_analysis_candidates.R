...
cands = readRDS("lncRNAs_selected_by_EN_april14.rds") #1000 runs of cross-validations using new updated dataset
...
get_survival_models = function(dtt){
  ...
  results_cox1 <- as.data.frame(matrix(ncol=22)) ; colnames(results_cox1) <- c("gene", "coef", "pval", "HR", "low95", "upper95", "cancer",
    ...
    "lnc_only_concordance", "clinical_only_concordance", "num_events", "perc_wevents", "anova_pval", "hr_adjusted", "pval_adjusted")
...
pdf("/u/kisaev/Jan2021/TCGA_candidates_survival_plots_final_cands_FULL_10year_PFI.pdf", width=6, height=5)
...
write.table(tcga_results1, file="/u/kisaev/Jan2021/SuppTable4_PFI.txt", quote=F, row.names=F, sep=";")
