...
#only keep lncRNAs with a median expression of at least 1
meds = apply(canc_data[,1:(ncol(canc_data)-5)],2, median)
z = which(meds==0)
...
x <- model.matrix( ~., trainlncs)
y <- Surv(trainlncs$time, trainlncs$status)
...
trainlncs = trainlncs[,c(which(colnames(trainlncs) %in% c(genes_keep,"time", "status")))]
justlncs = coxph(Surv(time, status)  ~ ., data = trainlncs)
...
saveRDS(cinds_clin, file="ALL_OV_pats_prebin_predictors_clin_cindeces_CV_100_March16_justlncsaswell_med1.rds")
saveRDS(cinds_combined, file="ALL_OV_prebin_predictors_combined_cindeces_CV_100_March16_justlncsaswell_med1.rds")
saveRDS(cinds_justlncs, file="ALL_OV_prebin_predictors_justlncs_cindeces_CV_100_March16_justlncsaswell_med1.rds")
saveRDS(genes_results, file="ALL_OV_pats_prebin_predictors_list_of_sig_genes_CV_100March16_justlncsaswell_med1.rds")
