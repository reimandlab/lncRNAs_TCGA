...
top3genes = as.character(cands$gene[7])
canc_data = canc_data[,c(which(colnames(canc_data) %in% top3genes), 2359:ncol(canc_data))]
canc_data = canc_data[,-c(2,5)] #dont need cancer and sex columns
...
train[,ncol(train)] = log1p(train[,ncol(train)])

#for(k in 10:ncol(train)){
k = ncol(train)
...
train = train[,-c(1, 4, 5, 6, 7, 8, 9)] #remove cancer, patient and sex columns, newtime and newstatus + stage + grade, model 2 just lncRNAs 
...
saveRDS(cinds, file="MODEL10_305_OV_pats_CAND_genes_CV_1000timesJan26.RDS")
