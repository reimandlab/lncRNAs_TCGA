...
detectable = subset(detectable, canc == "Panc-AdenoCA")
detectable$canc = "Pancreatic adenocarcinoma"

z = which(colnames(canc_data) %in% c(detectable$gene, "canc", "time", "status", "sex", "patient"))
canc_data = canc_data[,z]
#save clinical file
saveRDS(canc_data, file="PAAD_tcga_RNA_data_only_detectable_iPCAWG_lncs_mar21.rds")
...
lncs = as.list(colnames(canc_data)[1:349])
...
pdf("lnc_exp_correlation_bw_pcawg_tcga_mar21_PAAD.pdf", height=12, width=10)
...
saveRDS(canc_data, file="PAAD_tcga_RNA_data_only_detectable_iPCAWG_lncs_mar21_mostcorrelated_lncs.rds")
