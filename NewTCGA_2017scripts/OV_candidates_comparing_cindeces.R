...
colsskeep = c(which(str_detect(colss, "stage")), which(str_detect(colss, "grade")), which(str_detect(colss, "days")), 
  which(str_detect(colss, "followup")), which(str_detect(colss, "age")))
...
canc_data = canc_data[,-c(10,13)] #dont need cancer and sex columns
...
saveRDS(cindes_lncs_vs_clinical, file="cinds_OV_clinical_vs_all_lncRNAs_feb9_1000_runs.rds")
...
saveRDS(cindes_individual_lncs, file="cinds_OV_individual_lncRNAs_feb9_1000_runs.rds")
