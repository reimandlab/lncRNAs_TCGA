...
check_wilcoxon = function(PCG){
		data = d[,c(1:7, which(colnames(d) %in% PCG))]
		wilcoxon = wilcox.test(data[which(data$exp==1),8], data[which(data$exp==0),8], alternative="two.sided")$p.value
		return(wilcoxon)
	}
...
boxplot = function(PCG){
		data = d[,c(1:7, which(colnames(d) %in% PCG))]
		colnames(data)[8] = "PCG_Expression"
		g = ggboxplot(data, x = "exp", y="PCG_Expression", title = paste(colnames(d)[6], d$canc[1], "Expression Distribution"))
...
diffE <- function(d){
	...
	pdf(paste(colnames(d)[6], d$canc[1], "volcano.pdf", sep="_"), pointsize=8, height=9, width=10)
	...
	wilcoxon_pcgs = sapply(colnames(d)[8:ncol(d)], check_wilcoxon)
    ...
    print(paste("t after subset", dim(t)[1]))
