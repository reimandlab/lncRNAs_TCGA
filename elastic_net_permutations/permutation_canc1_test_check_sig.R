...
for(c in 1:3){ #<---- make function CHECK


function(canc, start, end)

	#now we have our one random data - we will need to generate 100 random datasets 
	...
	for(j in 1:4){ ###CHECK <--- make function 
		...
		for(i in 2:(ncol(train)-34)) {
    		train[,i] <- as.numeric(train[,i])
		}

		for(i in 2:(ncol(test)-34)) {
	...
	filename_cinds = paste(dat$Cancer[1], c, "test_cindices_permutations_1000CV_1000_no_fdr_ELASTICNET.rds", sep="_") #CHECK
	saveRDS(all_cinds, file=filename_cinds)

	filename = paste(dat$Cancer[1], c, "test_lncRNAs_selected_permutations_1000CV_1000_no_fdr_ELASTICNET.rds", sep="_")
...
#canc_datas_survival_results = llply(canc_datas, lasso_cv, .progress="text")
