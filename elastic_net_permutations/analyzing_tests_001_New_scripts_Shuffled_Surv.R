...
get_fdr = function(canc){
  ...
  cands_lnc = filter(cands, cancer == canc)
  genes = as.character(cands_lnc$gene)
  canc_dat$concordance = as.numeric(canc_dat$concordance)
  concs = canc_dat[,c("concordance")]
  concs$type = "fake"

  concs_real = as.data.frame(cands_lnc[,c("multi_model_concordance")])
  
  if(dim(concs_real)[1] > 0){

  print("are any genes the same in real and fake data = false positives")
  print("real gene length")
  print(length(genes))
  print(filter(canc_dat, gene %in% genes))
    
  concs_real$type = "real"
  colnames(concs_real)[1] = "concordance"
  concs_real$concordance = as.numeric(as.character(concs_real$concordance))

  concs = rbind(concs, concs_real)
  concs$concordance = as.numeric(concs$concordance)

  g = gghistogram(concs, x = "concordance",
   add = "mean", rug = TRUE,
   color = "type", fill = "type",
   palette = c("#00AFBB", "#E7B800"), title=canc)
...
pdf("real_vs_fake_full_model_concordances.pdf")
...
plot_real_vs_permutation = function(canc){
  ...
  canc_dat = as.data.table(filter(res, cancer == canc))
