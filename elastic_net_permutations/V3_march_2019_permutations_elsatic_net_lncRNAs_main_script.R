...
#in this version, shuffle all gene expression values across the board 
...
random_permutations = function(canc){ #main permutation cross-validation function, which basically runs elastic net function on 100 random datasets 
  ...
  run_res = replicate(2, main_elastic_net(dato))
...
llply(cancers, random_permutations, .parallel=TRUE)
print("done all cancers")
