...
saveRDS(new_results, file="TCGA_GTEX_ionchannels_ranked_wDifferences_Dec30_noFDR.rds")
...
new_results = filter(new_results, abs(median_difference) >= 0.1)
new_results = as.data.table(new_results)
dim(new_results)
#[1] 1399    9
...
get_name = function(gene){
	name = ucsc$HGNC.symbol[which(ucsc$hg19.ensGene.name2 == gene)]
	return(name)
}
...
#FINAL PLOT 1F
pdf("summary_gtex_tcga_updown_regulated_cancer_speicifc_or_not_ion_channels_dec30.pdf")
...
theme_bw() + xlab("Number of Cancer Types") + ylab("Count") + ggtitle("Summary of Up/Downregulated IonChannels \nbetween GTEx & TCGA")
