...
#TP53 mutations
tp53 = readRDS("TCGA_TP53_muts.rds")
...
getClinical <- function(df){
	...
	z = which(colnames(df) %in% c("patient", "ENSG00000189298", "OS", "OS.time", "Cancer"))
	ZKSCAN3 = df[,z]
  ...
  z = which(ZKSCAN3$patient %in% tp53$Tumor_Sample_Barcode)
  ZKSCAN3$tp53_mut = ""
  ZKSCAN3$tp53_mut[z] = "TP53_mut"
  ZKSCAN3$tp53_mut[-z] = "No_TP53_mut"

  #assign new tag to those that have ZKSCAN3 expression + TP53 mut
  z = which((ZKSCAN3$tp53_mut == "TP53_mut") & (ZKSCAN3$ZKSCAN3_tag ==1))
  ZKSCAN3$combo = ""
  ZKSCAN3$combo[z] = "HighZ_TP53_mut"
  ZKSCAN3$combo[-z] = "Else"

  if(dim(table(ZKSCAN3$combo)) > 1){

  	lncs = coxph(Surv(OS.time, OS)  ~ combo, data = ZKSCAN3)
  	row <- c("ZKSCAN3+Tp53_mut", summary(lncs)$coefficients[1,c(1,2,5)],  summary(lncs)$conf.int[1,c(3,4)], ZKSCAN3$Cancer[1])
    
    colnames(ZKSCAN3)[2] = "ZKSCAN3"

  	ZKSCAN3$OS.time = ZKSCAN3$OS.time/365
    order = c("Else", "HighZ_TP53_mut")
    ZKSCAN3$combo <- factor(ZKSCAN3$combo, levels = order)

  	fit <- survfit(Surv(OS.time, OS) ~ combo, data = ZKSCAN3)
         
          s <- ggsurvplot(
          title = paste("ZKSCAN3", ZKSCAN3$Cancer[1], "HR =", round(as.numeric(row[3]), digits=4)),
          fit, 
          xlab = "Time (Years)", 
          surv.median.line = "hv",
          ...
          legend.labs = c("Else", "HighZ_TP53_mut"),             # survfit object with calculated statistics.
...
pdf("ZKSCAN3_KM_plots_TCGA_cancers_TP53_mut_july4.pdf")
