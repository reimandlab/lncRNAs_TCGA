...
get_survival_models = function(dtt){
  results_cox1 <- as.data.frame(matrix(ncol=12)) ; colnames(results_cox1) <- c("gene", "coef", "HR", "pval", "low95", "upper95", "cancer", 
    "lnc_test_ph", 'global_test_ph', "sample_size", "num_events", "covariates_used")
  ...
  row <- c(colnames(newdat)[1], summary(lncs)$coefficients[1,c(1,2,5)],  summary(lncs)$conf.int[1,c(3,4)], dtt$Cancer[1], lnc_test_ph, global, 
    dim(dat)[1], length(which(dat$OS==1)), cols_used)
...
pdf("TCGA_candidates_survival_plots_final_cands_Feb6.pdf")
...
saveRDS(tcga_results1, file="TCGA_FMRE_diff_exp_PCGS_survival_results_wCovariates_Feb6.rds")
