...
getClinical <- function(df){
	...
	ZKSCAN3$OS = as.numeric(ZKSCAN3$OS)
 	ZKSCAN3$OS.time = as.numeric(ZKSCAN3$OS.time)
  	...
  	lncs = coxph(Surv(OS.time, OS)  ~ ZKSCAN3_tag, data = ZKSCAN3)
  	row <- c("ZKSCAN3", summary(lncs)$coefficients[1,c(1,2,5)],  summary(lncs)$conf.int[1,c(3,4)], ZKSCAN3$Cancer[1])
    
    colnames(ZKSCAN3)[2] = "ZKSCAN3"

  	ZKSCAN3$OS.time = ZKSCAN3$OS.time/365
  	fit <- survfit(Surv(OS.time, OS) ~ ZKSCAN3_tag, data = ZKSCAN3)
...
pdf("ZKSCAN3_KM_plots_TCGA_cancers_June14.pdf")
